...
# Allow quick-filter and selection
species_names = [f"{s['id']} - {s['name']}" for s in species_e]
selected_species = st.multiselect('Choose species to expose as sliders (select key nutrients/products)', species_names, default=species_names[:8])
species_map = {f"{s['id']} - {s['name']}": s for s in species_e}

st.markdown('### Slider bounds / fine-tuning')
...
if selected_species:
    for key in selected_species:
        ...
        min_v = cols[0].number_input(f'{sid} min', value=0.0, step=0.1, key=f'{sid}_min')
        max_v = cols[1].number_input(f'{sid} max', value=100.0, step=0.1, key=f'{sid}_max')
        default_v = cols[2].number_input(f'{sid} default', value=10.0, step=0.1, key=f'{sid}_def')
        # sanitize
        if max_v < min_v:
            max_v = min_v
...
if selected_species:
    ...
    for i, key in enumerate(selected_species):
        ...
        val = col.slider(s['name'], min_value=lo, max_value=hi, value=default_vals.get(sid, (lo+hi)/2), step=max((hi-lo)/100.0, 0.01), key=sid)
...
if selected_species and attempt_cobra:
    ...
    for key in selected_species:
        ...
        rid = st.text_input(f'Reaction for {sid}', value='', placeholder=default_guess, key=f'map_{sid}')
        if rid.strip():
...
if objective_choice in ['Mock product', 'TRY (titer/rate/yield)']:
    product_choice = st.selectbox('Select product species (for titer/rate/yield calculations)', ['-- none --'] + selected_species)
